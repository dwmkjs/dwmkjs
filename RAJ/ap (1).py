...
def frequency(C,transctions,K):
	...
	while True:
		...
		if L==[]:
			return Lpre
def comb(temp,data_list):
	temp1=[]
	for i in range(len(data_list)):
		if  data_list[i] not in temp:
			temp1.append(data_list[i])
	return temp1

def confidence(data_list):
	rules_list=[]
	print(len(data_list[1]))
	for i in range(len(data_list[1])):		
		for j in range(0,len(data_list[1])-1):
			z=0
			temp=[]
			for z in range(i,j):
				print (str(i)+" "+str(j))
				temp.append(data_list[1][z])
				print(temp)
				print(comb(temp,data_list[1]))
				print([data_list[0],temp,comb(temp,data_list[1])])
			if [data_list[0],temp,comb(temp,data_list[1])] not in rules_list:
				rules_list.append([data_list[0],temp,comb(temp,data_list[1])])
	print(rules_list)
			
confidence([1,["a","b","c","f"]])
